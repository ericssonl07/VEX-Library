...
Chassis::Chassis(MotorGroup* left, MotorGroup* right, vex::rotation * front_back_encoder, vex::rotation * left_right_encoder, vex::inertial * inertial_sensor,
                 ...
                 double initial_x, double initial_y, double initial_rotation, int thread_sleep): 
                    ...
                    odom(front_back_encoder, left_right_encoder, inertial_sensor, base_width, wheel_radius, thread_sleep),
...
void Chassis::basic_control(void * instance) {
    ...
    while (true) {
        double left_power = controller.Axis3.position();
        double right_power = controller.Axis2.position();
        chassis -> left -> spin(vex::fwd, left_power, vex::pct);
        chassis -> right -> spin(vex::fwd, right_power, vex::pct);
        vex::this_thread::sleep_for(chassis -> thread_sleep);
    }
}